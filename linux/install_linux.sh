...
# Get the project root directory (parent of linux/)
...
# Check if zenity is installed (recommended for GUI dialogs)
if ! command -v zenity &> /dev/null; then
    echo ""
    echo "⚠️  Warning: zenity is not installed"
    echo "   The revert dialog will work but won't have a GUI"
    echo "   Install zenity for better experience:"
...
# Read the hooks.json and replace FOLDER placeholder with absolute path to linux folder
...
echo "✓ All command paths have been updated to absolute paths in linux folder"
echo ""
